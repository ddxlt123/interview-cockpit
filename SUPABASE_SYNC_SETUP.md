...
Supabase 内置邮件服务仅用于试用，当前每个项目每小时最多发送 2 封认证邮件，而且只向项目成员邮箱发送。个人测试可以等待额度恢复后重试；长期使用应在 **Authentication → Emails → SMTP Settings** 配置自定义 SMTP，配置后再到 **Authentication → Rate Limits** 调整邮件额度。
